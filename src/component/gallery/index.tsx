...
export const Gallery = () => {
  ...
  const itemsToRender = useMemo(() => {
    if (status === "moving-right") {
      return CAROUSEL_ITEMS.slice(moveOption.srcIdx, moveOption.dstIdx + 1)
    }
    if (status === "moving-left") {
      return CAROUSEL_ITEMS.slice(moveOption.dstIdx, moveOption.srcIdx + 1)
    }

    const items = []
    // prev
    items.push(CAROUSEL_ITEMS[slide === 0 ? CAROUSEL_ITEMS.length - 1 : slide - 1])
    // current
    items.push(CAROUSEL_ITEMS[slide])
    // next
    items.push(CAROUSEL_ITEMS[slide === CAROUSEL_ITEMS.length - 1 ? 0 : slide + 1])
    return items
  }, [status, slide, moveOption])
      ...
      <div className="carousel-wrapper">
        ...
        <div className="carousel-scrollbar">
          <div
            ...
            style={{
              width: `${100 / CAROUSEL_ITEMS.length}%`,
