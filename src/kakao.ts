...
export const initializeKakao = (jsKey: string) => {
  ...
  if (document.getElementById("kakao-sdk-script")) {
    console.log("Kakao SDK 스크립트가 이미 존재합니다.");
    // 이미 로드되었지만 초기화되지 않은 경우 재초기화 시도
  ...
  script.src = "https://t1.kakaocdn.net/kakao_js_sdk/2.7.2/kakao.min.js"; // 카카오 SDK CDN 주소 (최신 버전 사용 권장)
  ...
  script.onload = () => {
    ...
    if (window.Kakao && !window.Kakao.isInitialized()) {
      ...
      console.error("Kakao SDK 로드 실패: window.Kakao 객체 없음");
    }
  };
